...
export function ResultSearch({ data }) {
  ...
  const base_url =
    "https://formafusionmg.ams3.cdn.digitaloceanspaces.com/formafusionmg/img/entreprises";
              ...
              <table className="w-full text-sm text-left border-collapse">
                <thead className="bg-slate-50 text-slate-600 font-medium border-b">
                  <tr>
                    ...
                    <th className="px-4 py-3 w-1/4">{t("training")}</th>
                    <th className="px-4 py-3 w-[15%]">{t("date")}</th>
                    <th className="px-4 py-3 w-[8%]">{t("company")}</th>
                    <th className="px-4 py-3">{t("status")}</th>
                    <th className="px-4 py-3">{t("projectType")}</th>
                    <th className="px-4 py-3">{t("modality")}</th>
                    <th className="px-4 py-3">{t("learners")}</th>
                    <th className="px-4 py-3">{t("trainers")}</th>
                  ...
                  {nombre[1].map((projet, i) => (
                    <tr key={i} className=" transition duration-300 border-b">
                      ...
                      <td className="px-4 py-3 text-base font-semibold">
                        <span>
                          {dayjs(projet.dateDebut).format("DD MMM YYYY")} -{" "}
                          {dayjs(projet.dateFin).format("DD MMM YYYY")}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-block px-3 py-1 rounded-full text-base ${
                            projet.modalite === "Présentielle"
                              ? "text-[#00b4d8] bg-[#00b4d8]/10"
                              : projet.modalite === "En ligne"
                              ? "text-[#fca311] bg-[#fca311]/10"
                              : projet.modalite === "Blended"
                  ...
                  {projet.apprs.length > 3 && (
                    <div className="w-8 h-8 bg-gray-600 text-white rounded-full flex items-center justify-center text-[13px] font-bold tracking-tight shadow-md border-2 border-white">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex -space-x-3 rtl:space-x-reverse cursor-pointer">
                          {projet.formateurs.slice(0, 3).map((formateurs, idx) => (
                            <div
                              key={formateurs.idFormateur || idx}
                              onClick={() =>
                                hanldeOpenDrawer("formateur", formateurs.idFormateur)
                              }
                            >
                      ...
                      <td className="px-4 py-3">
                        <button
                          onClick={() =>
                            hanldeOpenDrawer("presence", projet.idProjet, projet.idCfp_inter)
                          }
